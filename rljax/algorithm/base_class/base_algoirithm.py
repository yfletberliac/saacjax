...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        if not hasattr(self, "buffer"):
            if use_per:
                ...
                self.buffer_challenger = PrioritizedReplayBuffer(
                    buffer_size=buffer_size,
                    state_space=state_space,
                    action_space=action_space,
                    gamma=gamma,
        ...
        if update_interval_target:
            ...
            self._update_challenger = jax.jit(partial(soft_update, tau=1.0))
        else:
            self._update_target = jax.jit(partial(soft_update, tau=tau))
            self._update_challenger = jax.jit(partial(soft_update, tau=tau_challenger))
    ...
    def step(self, env, state):
        ...
        self.buffer_challenger.append(state, action, reward, mask, next_state, done)
