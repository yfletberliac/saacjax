...
class SACAGAC(OffPolicyActorCritic):
    name = "SACAGAC"
    ...
    def update(self, writer=None):
        ...
        # Update critic challenger.
        self.opt_state_critic_challenger, self.params_critic_challenger, loss_critic_challenger, loss_critic_challenger_log = optimize(
            self._loss_critic_challenger,
            self.opt_critic_challenger,
            self.opt_state_critic_challenger,
            self.params_critic_challenger,
            self.max_grad_norm,
            params_critic_challenger_target=self.params_critic_challenger_target,
        ...
        _, reward_c = loss_critic_challenger_log
        ...
        self.opt_state_challenger, self.params_challenger, loss_challenger, _ = optimize(
            ...
            self.max_grad_norm,
            params_critic_challenger=self.params_critic_challenger,
            state=state,
            log_alpha=self.log_alpha,
            **self.kwargs_actor,
        )
        ...
        # Update challenger target network.
        self.params_critic_challenger_target = self._update_target(self.params_critic_challenger_target,
                                                                   self.params_critic_challenger)

        if writer and self.learning_step % 10000 == 0:
            writer.add_scalar("loss/critic", loss_critic, self.learning_step)
            writer.add_scalar("loss/critic_challenger", loss_critic_challenger, self.learning_step)
    ...
    def _calculate_target(
        ...
        # TODO take logpi of action taken
        next_log_pi = self._calculate_log_pi(next_action, next_log_pi)
        next_q -= jnp.exp(log_alpha) * next_log_pi
        return jax.lax.stop_gradient(reward + (1.0 - done) * self.discount * next_q)

    @partial(jax.jit, static_argnums=0)
    def _calculate_target_challenger(
            self,
            params_critic_challenger_target: hk.Params,
            log_alpha: jnp.ndarray,
            state: np.ndarray,
            done: np.ndarray,
            next_state: np.ndarray,
        ...
        next_q = self._calculate_value(params_critic_challenger_target, next_state, next_action)
        next_log_pi = self._calculate_log_pi(next_action, next_log_pi)
        next_q -= jnp.exp(log_alpha) * next_log_pi
        # reward_c = (jnp.ones((len(state), self.num_constraints)) - state[:, -self.num_constraints:]).mean(axis=-1, keepdims=True)
        reward_c = (jnp.ones((len(state), 2)) - state[:, -self.num_constraints:(-self.num_constraints+2)]).mean(axis=-1, keepdims=True)
        # reward_c = (jnp.ones((len(state), 1)) - state[:, -self.num_constraints]).mean(axis=-1, keepdims=True)
        return jax.lax.stop_gradient(reward_c + (1.0 - done) * self.discount * next_q), jax.lax.stop_gradient(
            reward_c.mean())
    ...
    @partial(jax.jit, static_argnums=0)
    def _loss_critic_challenger(
            self,
            params_critic_challenger: hk.Params,
            params_critic_challenger_target: hk.Params,
        ...
        next_action, next_log_pi = self._sample_action(params_challenger, next_state, *args, **kwargs)
        target, reward_c = self._calculate_target_challenger(params_critic_challenger_target, log_alpha, state, done,
                                                             next_state,
                                                             next_action, next_log_pi)
        q_list = self._calculate_value_list(params_critic_challenger, state, action)
        loss_critic_challenger, abs_td = self._calculate_loss_critic_and_abs_td(q_list, target, weight)
        return loss_critic_challenger, [abs_td, reward_c]
    ...
    def _loss_challenger(
        ...
        mean_q_challenger = self._calculate_value(params_critic_challenger, state, action_challenger).mean()
        return jax.lax.stop_gradient(jnp.exp(log_alpha)) * mean_log_pi_challenger - mean_q_challenger, None
