...
class SACVAR(OffPolicyActorCritic):
    name = "SACVAR"
    ...
    def update(self, writer=None):
        ...
        reward_c = (jnp.ones((len(state), 2)) - state[:, -self.num_constraints:(-self.num_constraints + 2)]).mean()
        # reward_c = (jnp.ones((len(state), 1)) - state[:, -self.num_constraints]).mean()
    ...
    def _loss_challenger(
        ...
        q = self._calculate_value(params_critic, state, action_challenger)
        return jax.lax.stop_gradient(jnp.exp(log_alpha)) * mean_log_pi_challenger \
               - 0.1 * jnp.square(q-q.mean()).mean(), None
